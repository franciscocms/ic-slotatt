...
def sample_clevr_scene(N):
    ...
    size_masked = pyro.sample(f"size_mask", dist.Categorical(probs=torch.tensor([1/len(size_mapping) for _ in range(len(size_mapping))])).to_event(1), obs=size, mask=mask)

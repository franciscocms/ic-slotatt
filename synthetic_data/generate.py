...
from pillowrender import render, render_annot
...
def model(scene_id):
  ...
  dot_annot = render_annot(locx_obj, locy_obj, BACKGROUND_COLOR)
  return img, dot_annot, scene_dict
...
img_dir = '' # path for dataset images
dot_annot_dir = '' # path for dataset dot annotations
dirs = [metadata_dir, img_dir, dot_annot_dir]
for d in dirs:
...
for scene_id in range(NSCENES):
  ...
  img, dot_annot, scene_dict = model(scene_id)
  ...
  dot_annot.save(f"{dot_annot_dir}/{str(scene_id).zfill(5)}.png")

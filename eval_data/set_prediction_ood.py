...
def main():    
    ...
    for seed in seeds: 
        ...
        ood_img_dir = os.path.join(main_dir, "synthetic_data", "new_ood_samples")

        for img_path in glob.glob(os.path.join(ood_img_dir, "*.png")):

            #if img_path.split('/')[-1].split('.')[0].split('_')[0] == "clevr":
            #if img_path == img_path:
            if img_path.split('/')[-1].split('.')[0] == '00028':
                ...
                if img_path.split('/')[-1].split('.')[0].split('_')[0] == "clevr":
                    sample = sample[0, :3, :, :].unsqueeze(0)
                    sample = F.interpolate(input=sample, size=(128, 128), mode='bilinear', antialias=True)
                    sample = torch.clamp(sample, 0., 1.)
               
                sample = sample.to(device)
                sample_id = img_path.split('/')[-1].split('.')[0]
                
                output_img_dir = os.path.join(ood_img_dir, sample_id)
                if not os.path.isdir(output_img_dir): os.mkdir(output_img_dir)
                
                if img_path.split('/')[-1].split('.')[0].split('_')[0] == "clevr":
                    # save interpolated image to compare with rendered!!
                    plt.imshow(sample[0].permute(1, 2, 0).numpy())
                    plt.savefig(f'{output_img_dir}/resized_img.png')
                    plt.close()            
                ...
                if params['inference_method'] == 'score_resample' and params['proposals'] == 'data_driven':
                    ...
                    for vars in latent_vars: # 'vars' represent the group of latent variables associated with the same object
                        ...
                        shape_list = [tracking_dict[t][f"shape_{vars_id}"] for t in range(len(traces)) if t in tracking_dict]
                        color_list = [tracking_dict[t][f"color_{vars_id}"] for t in range(len(traces)) if t in tracking_dict]
                        print("\n",{"ball": len([i for i in shape_list if i == 'ball']),
                        ...
                        #     "blue": len([i for i in color_list if i == 'blue'])
                        #     },"\n")
                        ...
                        plt.savefig(f'{output_img_dir}/traces_overlay_vars_{vars_id}.png')
                        plt.close()
                        ...
                        for name, site in resampled_model_trace.nodes.items():
                            ...
                            if name == 'image': # and vars == latent_vars[-1]:
                                ...
                                plt.savefig(f'{output_img_dir}/pred_vars_{vars_id}.png')
                                plt.close()
                    ...
                    plt.imshow(overlay_img.permute(1, 2, 0).numpy())
                    plt.savefig(f'{output_img_dir}/IS_traces_overlay.png')
                    plt.close()
                    ...
                    plt.savefig(f'{output_img_dir}/IS_pred.png')
                    plt.close()
